...
export default function AddUserInfoPage() {
  ...
  const handleSubmitForm = async (e) => {
    ...
    if (!first_name.trim()) newErrors.first_name = "Vui lòng điền đầy đủ họ tên.";
    ...
    if (!phoneNum || phoneNum.trim() === "") newErrors.phoneNum = "Số điện thoại không được để trống";
    else if (!/^0\d{9}$/.test(phoneNum)) newErrors.phoneNum = "Số điện thoại 10 số bắt đầu bằng số 0";
    ...
    try {
      ...
      if (!updatedUser.role) updatedUser.role = String(user?.role || "DRIVER").toUpperCase();
      ...
      navigate("/driver", { replace: true, state: { from: "add-info-success" } });
    } catch (err) {
  ...
  return (
    <div className="background">
      <div className="!min-h-screen !w-full !flex !items-center !justify-center !px-4 !py-8">
        ...
        >
          <div className="!mb-4">
            <h1 className="!text-center !font-bold !text-[25px] !text-[#00ffc6]">Vui lòng nhập thông tin</h1>
          </div>

          {/* Họ và tên */}
          <div className="!mb-5">
            <label className="!block !text-[#eaeaea] !font-semibold !mb-2">Họ và tên</label>
            <div className="!flex !w-full !gap-2">
              <input
                ...
                onFocus={handleFocus}
                className={`${baseInput} !flex-[1.7] ${errors.last_name ? "!border-red-500 !ring-2 !ring-red-500" : ""}`}
              />
              <input
                name="first_name"
                type="text"
                ...
                onChange={handleChangeValue}
                onFocus={handleFocus}
                className={`${baseInput} !flex-1 ${errors.first_name ? "!border-red-500 !ring-2 !ring-red-500" : ""}`}
              />
            </div>
            {(errors.last_name || errors.first_name) && (
              <div className="!mt-1 !text-red-500 !text-sm">{errors.last_name || errors.first_name}</div>
            )}
          </div>

          {/* Giới tính */}
          <div className="!mb-5">
            <label className="!block !text-[#eaeaea] !font-semibold !mb-2">Giới tính</label>
          ...
          {/* Ngày sinh */}
          <div className="!mb-5">
            <label className="!block !text-[#eaeaea] !font-semibold !mb-2">Ngày tháng năm sinh</label>
            <input
              name="dob"
              type="date"
              lang="vi"
              onChange={handleChangeValue}
              onFocus={handleFocus}
              className={`${baseInput} ${errors.dob ? "!border-red-500 !ring-2 !ring-red-500" : ""}`}
            />
            {errors.dob && <div className="!mt-1 !text-red-500 !text-sm">{errors.dob}</div>}
          </div>

          {/* SĐT */}
          <div className="!mb-4">
            <label className="!block !text-[#eaeaea] !font-semibold !mb-2">Số điện thoại</label>
            <input
              name="phoneNum"
              type="text"
              ...
              onChange={handleChangeValue}
              onFocus={handleFocus}
              className={`${baseInput} ${errors.phoneNum ? "!border-red-500 !ring-2 !ring-red-500" : ""}`}
            />
            {errors.phoneNum && <div className="!mt-1 !text-red-500 !text-sm">{errors.phoneNum}</div>}
          </div>
