  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800">
      ...
      <section id="subscription" className=" py-20 bg-gradient-to-r from-blue-600/20 to-green-600/20 backdrop-blur-sm">
        ...
        {loading && <div className="text-center text-white">Đang tải các gói...</div>}
            ...
            <div className="flex-1 max-w-2xl">
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20">
                ...
                <div className="space-y-4 text-white/90">
                  <p><strong>Loại thanh toán:</strong> {plans[activeFeature]?.billingType === 'MONTHLY_SUBSCRIPTION' ? 'Theo tháng' : plans[activeFeature]?.billingType}</p>
                  <p><strong>Phí hàng tháng:</strong> {formatCurrency(plans[activeFeature]?.monthlyFee)}</p>
                  <p><strong>Giá mỗi kWh:</strong> {formatCurrency(plans[activeFeature]?.pricePerKwh)}</p>
                  <p><strong>Giá mỗi phút:</strong> {formatCurrency(plans[activeFeature]?.pricePerMinute)}</p>
                  <p><strong>Quyền lợi khác:</strong> {plans[activeFeature]?.benefits}</p>
                </div>
              </div>
            </div>
          </div>
