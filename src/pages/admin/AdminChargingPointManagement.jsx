...
const AdminChargingPointManagement = () => {
                ...
                stationsData.map(async (station) => {
                    ...
                    if (station.stationId) {
                        try {
                            ...
                            chargingPoints =
                                chargersResponse?.data?.result || chargersResponse?.result || [];
    ...
    const handleSaveChanges = async () => {
        try {
            ...
            toast.success(`Đã lưu thay đổi cho trụ "${editModal.point.name}" thành công!`);
            setEditModal({ show: false, point: null, stationId: null });
            await loadStationsData(false);
        } catch (err) {
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">
                                    {stations.reduce((sum, s) => sum + s.chargingPoints.length, 0)}
                                </p>
                            </div>
                        </div>
                    </div>
                                ...
                                {!station.chargingPoints || station.chargingPoints.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {station.chargingPoints.map((point) => {
                                            ...
                                            return (
                                                <div key={point.pointId} className="border-2 border-gray-200 rounded-lg p-4 hover:border-[#22c55e] transition-colors relative">
                                                    {/* Status Badge */}
